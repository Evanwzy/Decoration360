...
- (void)AVCaptureVideoOrientationLandscapeLeft:(UIInterfaceOrientation)interfaceOrientation
{
     [CATransaction begin];
...
-(void)Captureimage
{
    ...
    [captureOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:
     ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
         ...
         float w =width *scale_screen;
         if (w >=640) {
             image = [[UIImage alloc]initWithCGImage:t_image.CGImage scale:2.025 orientation:UIImageOrientationRight];
         }else {
             image = [[UIImage alloc]initWithCGImage:t_image.CGImage scale:0.75 orientation:UIImageOrientationRight];
         }
         [t_image release];
//         [[NSNotificationCenter defaultCenter] postNotificationName:@"GETPICTURE" object:nil];
